"""Custom PyTorch functions for DWT and IDWT (1D, 2D, 3D).

This module implements the core wavelet transform functions as custom PyTorch
autograd functions, enabling efficient forward and backward passes.
...
class DWTFunction_1D(Function):
    """1D Discrete Wavelet Transform function."""
...
class IDWTFunction_1D(Function):
    """1D Inverse Discrete Wavelet Transform function."""
...
class DWTFunction_2D(Function):
    """2D Discrete Wavelet Transform function.

    Returns four subbands: LL (low-low), LH (low-high), HL (high-low), HH (high-high).
    """
...
class DWTFunction_2D_tiny(Function):
    """2D DWT that only returns the low-frequency component (LL).

    Used in WaveCNet for efficient downsampling without preserving high-frequency details.
    """
...
class IDWTFunction_2D(Function):
    """2D Inverse Discrete Wavelet Transform function.

    Reconstructs the original signal from four subbands.
    """
...
class DWTFunction_3D(Function):
    """3D Discrete Wavelet Transform function.

    Returns eight subbands: LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH.
    The naming convention uses L for low-frequency and H for high-frequency
    in the order of (depth, height, width) dimensions.
    """
    ...
    def forward(
        ...
        # Apply wavelet transform along height (dim -2)
        L = torch.matmul(matrix_Low_0, input)
        H = torch.matmul(matrix_High_0, input)

        # Apply wavelet transform along width (dim -1) and transpose for depth
        ...
        # Apply wavelet transform along depth (dim -3 after transpose)
    ...
    @staticmethod
    def backward(
        ctx, grad_LLL, grad_LLH, grad_LHL, grad_LHH, grad_HLL, grad_HLH, grad_HHL, grad_HHH
    ):
        (
            matrix_Low_0,
            matrix_Low_1,
        ...
        # Backward through depth dimension
        grad_LL = torch.add(
        ...
        # Backward through width dimension
        grad_L = torch.add(
            torch.matmul(grad_LL, matrix_Low_1.t()),
            torch.matmul(grad_LH, matrix_High_1.t()),
        )
        ...
        # Backward through height dimension
        grad_input = torch.add(
            torch.matmul(matrix_Low_0.t(), grad_L),
            torch.matmul(matrix_High_0.t(), grad_H),
        )
...
class IDWTFunction_3D(Function):
    """3D Inverse Discrete Wavelet Transform function.

    Reconstructs the original 3D signal from eight subbands.
    """
    ...
    def forward(
        ...
        # Inverse transform along depth dimension
        input_LL = torch.add(
        ...
        # Inverse transform along width dimension
        input_L = torch.add(
            torch.matmul(input_LL, matrix_Low_1.t()),
            torch.matmul(input_LH, matrix_High_1.t()),
        )
        ...
        # Inverse transform along height dimension
        output = torch.add(
    ...
    def backward(ctx, grad_output):
        ...
        # Forward through height dimension
        grad_L = torch.matmul(matrix_Low_0, grad_output)
        grad_H = torch.matmul(matrix_High_0, grad_output)

        # Forward through width dimension
        ...
        # Forward through depth dimension
